...
function generateUuidChecked(){
	...
	if(isChecked){
		...
		d.className = "";
		$('#gatewayType option').eq(0).prop('selected', true);
		return true;
	}else{
		enableUuidInput()
		return false;
...
function provideUuidChecked(){
	...
	if(isChecked){
		...
		$('#gatewayType option').eq(1).prop('selected', true);
		return true;
	}else{
		return false;
	}
...
$(document).ready(function () {
	document.getElementById("generateUuid").checked = true;
	$("uuidInput").hide();
	disableUuidInput()
	
	
	$(document).on('change', '#provideUuid', function () {
		provideUuidChecked();
	});
	
	$(document).on('change', '#generateUuid', function () {
		generateUuidChecked();
	...
	$(document).on('change', '#preview-logo-input', function () {
		closeerror();
		if (!calculateFileSize('preview-logo-input', 12072000)) {
			displayerror('<li>Site map image can not exceed 12 megabytes.</li>');
        return;
    }
